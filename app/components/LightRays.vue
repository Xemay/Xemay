...
float rayStrength(vec2 raySource, vec2 rayRefDirection, vec2 coord,
  ...
  float viewportExtent = max(iResolution.x, iResolution.y);
  float maxDistance = viewportExtent * rayLength;
  float lengthFalloff = clamp((maxDistance - distance) / maxDistance, 0.0, 1.0);

  float fadeFalloff = clamp((viewportExtent * fadeDistance - distance) / (viewportExtent * fadeDistance), 0.5, 1.0);
